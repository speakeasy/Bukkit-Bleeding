...
public class InventoryClickEvent extends InventoryActionEvent implements Cancellable {
    ...
    public InventoryClickEvent(InventoryView what, SlotType type, int slot, boolean right, boolean shift) {
        this(what, type, slot, right ? (shift ? ClickType.SHIFT_RIGHT : ClickType.RIGHT) : (shift ? ClickType.SHIFT_LEFT : ClickType.LEFT));
    }

    public InventoryClickEvent(InventoryView what, SlotType type, int slot, ClickType action) {
        super(what, action);
        this.slot_type = type;
    ...
    public InventoryClickEvent(InventoryView what, SlotType type, int slot, int key) {
        this(what, type, slot, ClickType.NUMBER_KEY);
        this.hotbarKey = key;
    }
